...
void Port_Init(const Port_ConfigType * ConfigPtr )
{
    ...
    switch (Port_Channel_Config[index].port_num)
        ...
        if ((Port_Channel_Config[index].port_num == 2) && (Port_Channel_Config[index].pin_num <= 3)) /* PC0 to PC3 */
        {
            continue;
        ...
        if(Port_Channel_Config[index].direction == PORT_PIN_IN)
        {
            ...
            else
           {
            ...
            if(Port_Channel_Config[index].resistor == PULL_DOWN)
            {
            ...
            else
           {
            ...
            if((Port_Channel_Config[index].resistor != PULL_DOWN) && (Port_Channel_Config[index].resistor != PULL_UP))
            {
        ...
        *(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_CTL_REG_OFFSET) &= ~(0x0000000F << (Port_Channel_Config[index].pin_num * 4));
        ...
        if (Port_Channel_Config[index].mode == PORT_PIN_MODE_ADC)
        {
        ...
        if ((Port_Channel_Config[index].mode != PORT_PIN_MODE_ADC) && (Port_Channel_Config[index].mode != PORT_PIN_MODE_DIO))
        {
    ...
    if (FALSE == error)
    {
        ...
        if (Port_Channel_Config[Pin_Index].mode == PORT_PIN_MODE_DIO)
                ...
                else
               {
                 ...
                 if (Port_Channel_Config[Pin_Index].mode == PORT_PIN_MODE_ADC)
                {
                /* Setup the pin mode as ADC */
                 ...
                 else
                {
                 ...
                 if ((Port_Channel_Config[Pin_Index].mode != PORT_PIN_MODE_ADC) && (Port_Channel_Config[Pin_Index].mode == PORT_PIN_MODE_DIO))
                {
                /* Setup the pin mode as Any Alternative Mode */
...
void Port_SetPinDirection(Port_PinType Pin_Index, Port_PinDirectionType Direction)
{
    ...
    if (FALSE == error)
    {
        ...
        if (Port_Channel_Config[Pin_Index].direction == PORT_PIN_OUT)
        {
         ...
         if (Port_Channel_Config[Pin_Index].direction == PORT_PIN_IN)
        {
            /* Clear the corresponding bit in the GPIODIR register to configure it as input pin */
            CLEAR_BIT(
                    *(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIR_REG_OFFSET),
